...
DX12Hook::DX12Hook()
{
	...
	commandQueue.store(nullptr, std::memory_order_relaxed);
	mainSwapChain.store(nullptr, std::memory_order_relaxed);
}
...
{
	return commandQueue.load(std::memory_order_relaxed);
}

std::atomic<IDXGISwapChain*>& DX12Hook::GetMainSwapChain()
{
...
{
	GUIImplementation::OnPresent((IDXGISwapChain3*)pSwapChain);
}
...
void DX12Hook::OnExecuteCommandLists(ID3D12CommandQueue* pQueue, UINT NumCommandLists, ID3D12CommandList* ppCommandLists)
{
	...
	commandQueue.compare_exchange_strong(expected, pQueue, std::memory_order_relaxed);
}

HRESULT __stdcall DX12Hook::Present(IDXGISwapChain* pSwapChain, UINT SyncInterval, UINT Flags)
{
	std::atomic<IDXGISwapChain*>& mainSwapChain = DX12Hook::Get()->GetMainSwapChain();
	IDXGISwapChain* expected = nullptr;
	mainSwapChain.compare_exchange_strong(expected, pSwapChain, std::memory_order_relaxed);
	IDXGISwapChain* pMainSwapChain = mainSwapChain.load(std::memory_order_relaxed);

	uintptr_t addr = DX12Hook::Get()->GetPresentAddress();
	assert(addr != 0);

	if (pSwapChain == pMainSwapChain)
